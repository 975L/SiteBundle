...
The tagline — and anything else a site says under its own name — is the blocks of the `navbar-brand` `Menu`, written and translated on the Menus screen like any other block, in place of the `site-tagline` setting a config entry said in one language to everyone. `c975l:site:content:adopt-config-texts` moves an existing one across, as the first block of that menu.
...
They were four config entries (`email-text-*`) until 02/09/2026. A config entry carries no language, so a bilingual site said them in one language to everyone — which is also why they shipped **empty**: ready-made French paragraphs read as French on a site that isn't. One row per language answers that, so they now ship with real wording in `fr`/`en`/`es`; a site that wants none empties the row, and `ensure` never puts back the wording of a template that already exists. `c975l:site:content:adopt-config-texts` moves what a site already wrote into the row of its own language — a one-off, meant to be removed from `deploy.yml` once every site has run it (see `TODO-a-retirer-apres-deploiement.md`).
