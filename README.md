...
- **Two tabs can't overwrite each other**: `version` is Doctrine's optimistic lock, so a save made from an edit screen opened before another save of the same page is refused, the editor's input kept on screen. The edit screen's title names the page and the language written in, to tell open tabs apart
...
![SiteBundle blocks](.github/images/SiteBlocks.png)

One tile per kind, captured on the showcase at [bundles.975l.com](https://bundles.975l.com/pages/blocks/Site) - a kind with several variants shows only its first one, and a kind with no example there has no tile. Colors are the showcase's own theme, not what a site with its own theme renders.
...
| --- | --- | --- |
| `twig_content` | `label.category_twig` | Includes an existing Twig template by its path (`templatePath`, e.g. `pages/details/project.html.twig`) - a general-purpose "drop this template in here" block, not limited to the [collection item detail pages](#item-detail-pages) use case that motivated it. The path must be one of **your own** templates, under the app's `templates/` directory: the installed bundles' templates are refused, both by the form and at render time (see `TwigContentTemplateChecker`), a path typed in the back office being no basis for deciding what the site renders. |
...
Each kind also ships a **silhouette**, a CSS-drawn miniature of its layout (`sass/block-thumbs.scss`, compiled to `public/css/block-thumbs.min.css`) shown by UiBundle's visual block picker so a kind is recognisable before it is added. The sheet is contributed to the back-office alone, through the `ui.management_stylesheet` tag on `StylesheetProvider` — a site drawing the same silhouettes on a public page contributes it through its own provider, rather than every site carrying it on every page. A kind left undrawn falls back to UiBundle's default frame, which `StylesheetProviderTest` guards against by reading the pickable kinds off `config/services.yaml`.
...
`menu_link` resolves to a relative URL (`UrlGeneratorInterface::generate()`), fine for `navbar`/`footer` and rewritten against `site-url` inside an email by `emails/fullLayout.html.twig` (see [Email templates](#email-templates)) — so an `email-header`/`email-footer` link does reach the live site, as long as `site-url` is filled: an empty one leaves the path relative and the link dead in the mailbox.
...
| Columns | A grid, one column per group of links, the blocks placed before the first group stacking in a first, wider one (`.menu-items--columns`) |

The three classes **retune those very tokens** on the `.menu-items` element rather than writing `flex-direction` themselves (`sass/_footer.scss`), so one class is enough for both the wrapper and its `.blocks` child — `Block` additionally resetting `--footer-group-flex` to `0 0 auto` since a group's flex basis sits on the main axis — the width a theme gives a group to make it a column of a row footer would become a height once the footer stacks, laying the groups out that tall each instead of on their own content — and it beats what the theme left on `:root` — a declaration on the element always wins over an inherited one. Leaving the select on its placeholder stores `null` and adds no class at all, which is what every menu saved before the field existed holds: **a site that already picked its layout in `themes/site.css` keeps rendering exactly as it did**, and only starts following the backoffice once an admin picks a style there. Anything but the three known values is stored as `null` (`Menu::setStyle()`) — the value ends up in a class name, and an unknown one would only ever name a rule no stylesheet carries.

*Columns* retunes that pair like the other two do, and writes a grid on top of it: its first column holds several blocks the markup gives no wrapper for — a `menu_group` only ever takes links (see below), so that column is drawn by the layout instead of by a block around them. The blocks placed **before the first group** stack in it, one per row, which is how a footer opens on its site's name and its social links; each group then spans every row of a column of its own (`grid-row: 1 / -1`). Both track lists are read through a token with an inline fallback — `--footer-grid-columns` (`1.6fr repeat(3, 1fr)`) and `--footer-grid-rows` (`auto auto`, two rows for those two opening blocks) — so a footer counting another number of groups, or stacking another number of blocks in that first column, is retuned from `themes/site.css` without touching the style. A group laid out as a column inside it hangs from its column's edge rather than from its middle (`.blocks-group--column`), a list of links of uneven lengths reading as a ragged block otherwise — a group left as a row still centers its blocks on their line. Written mobile first like the rest of the bundle's sass, the stack every footer falls back to is the base and the grid only comes from 768px up, `--footer-group-flex` being reset there for the same reason *Block* resets it.
...
A footer often needs more than one line: its social links on one, its legal links on the next. Any menu that takes the full picker (`footer` and both email locations, i.e. everything but the `navbar`) can therefore hold **`menu_group`** blocks — a chromeless wrapper around several other blocks, which takes a line of its own inside the footer's flex row (`footer .menu-items .blocks-group { flex: var(--footer-group-flex, 0 0 100%) }`, `sass/_footer.scss` — the token is what lets a theme turn each group into a column of the row instead, and what the *Block* style resets when it stacks them). How the group itself lays its blocks out — row or column, and how they're justified — is picked on the group, UiBundle's `BlockGroupType` being reused as is.
...
Items are moved in or out of a group by drag and drop, like any container's slots. A group can't hold another group, so the tree is always one level deep, which is what `Footer.html.twig` walks when it checks whether a "Copyright" link is already there. Editing a group, a link or any other block (the `navbar-brand` tagline included) invalidates the cached menus through `MenuCacheInvalidationListener`, exactly as editing the `Menu` row does: a `Block` doesn't know which owner holds it, so every kind is watched.
...
A `menu_link` also has a `visibility` select (`MenuLinkType::VISIBILITIES`): `all` (the default, and what a link saved before the option existed reads as), `guests` (signed-out visitors only — a "Sign in" link) or `members` (signed-in visitors only — an account link). **Hidden in CSS rather than skipped at render, because the menu's html is cached once and shared by every visitor**: the link is always written, with a `.menu-item--guests`/`.menu-item--members` marker class, and `layout.html.twig` appends `is-guest` or `is-member` to `bodyClasses` from `app.user`, which `sass/_menu.scss` turns into a `display: none` (screen readers included). The class is on `<body>`, which Turbo swaps on every navigation, not on `<html>`, which it keeps. It holds as long as the page html itself is not shared-cached (no `HttpCache`/reverse proxy storing a page for everyone - neither this bundle nor the scaffold sets one up); a site layout that does not extend `@c975LSite/layout.html.twig` has to write the same class, or every link shows to everyone. In an email (`email-header`/`email-footer`) there is no visitor to tell apart, so `emails/header.html.twig` and `emails/footer.html.twig` leave a guests/members link out.
...
A **dark counterpart of the logo** can be uploaded on the `logo-on-dark` role of [Site graphics](#site-graphics), for a brand mark that does not read on a dark page. Both images are then written out — on the menu brand and on the fallback bar alike — and the stylesheet paints one of them, `.menu-logo__on-light` and `.menu-logo__on-dark` following the very selectors the dark palette is declared on (`sass/_menu.scss`): which of the two applies depends on the visitor's own system when `theme-mode` is `auto`, something no template can know. The dark one is written `loading="lazy"` and without a `fetchpriority`, and a browser leaves a hidden lazy image alone until it is shown, so a light-themed visitor never downloads it. **Both carry the same `alt`**, emptied by `site-navbar-show-name` exactly as the single logo's always was: `display: none` already takes one of the two out of the accessibility tree, so naming only one would leave the home link unnamed on the theme showing the other. A site that uploaded only one of the two — either one — writes that single image, unclassed and painted on both grounds, exactly as before.
...
The tagline — and anything else a site says under its own name — is the blocks of the `navbar-brand` `Menu`, written and translated on the Menus screen like any other block, in place of the `site-tagline` setting a config entry said in one language to everyone.

They are rendered by `Blocks` inside a `.menu-site-tagline` wrapper, so they arrive with a page section's own markup: `sass/_menu.scss` neutralizes the section gutter and measure there, plus the lead-in paragraph's scale and accent bar for the `text_hook` kind a tagline is usually written as — each of them a custom property with a fallback, which is what lets them be turned off rather than fought with `!important`. Style `.menu-site-tagline` in your own SCSS if you need to adjust it. It is not shown on a `fixed` navbar, which has to stay on the height `<body>` gets back from it.
...
This bundle's own page links are rewritten into the language the page around them is being read in — `PageLinkLocalizer`, answering UiBundle's `InternalLinkLocalizerInterface`, which every block renders its links through. A `/pages/nos-ateliers` written in a portfolio card, a call to action or inside a rich text is read as `/en/pages/nos-ateliers` on the English page, and left exactly as it is for a page nobody translated — a localised url of one would answer 404. The paths themselves come from `PagePublicUrlResolver`, so a link to the home page lands on `/en/` rather than on the `/en/pages/home` that only ever redirects there. A `page:12#services-34` or `route:shop_index` target picked in a block's link field is turned into its url in every language, the writing one included, through `MenuExtension::getMenuLinkUrl()`.
...
The navbar carries a **language menu** (`General:Languages`, rendered inside the bar's items so it follows the mobile dropdown, and on the fallback bar too). It offers the languages the page being read was really written in — the same list the `hreflang` group is built from — so it never links to a url that answers 404, and it renders nothing at all on a site declaring one language or on a page nobody translated. On a screen with no `Page` behind it — a shop listing, a product sheet, a basket — it offers instead the languages that screen itself answers in (`screen_languages()`, ConfigBundle's `LocalizedUrlGenerator::screenLanguages()`), and nothing at all where the screen answers in one language alone. A template declares the `Page` the menu rewrites the url of by setting `navbarPage`, which `pages/page.html.twig` leaves null on a collection item's detail view — the parent Page's languages name the parent's url, not the item's — and in preview, whose public urls would take the editor out of it.

Each entry points at the **bare url carrying `?_locale=xx`**, never at that language's own `/en/...`. A localised route already says which language it answers in, so ConfigBundle's `LocaleListener` returns early there without touching the session: a link straight to the localised url would serve that one page in that language and hand the writing language back on the very next click — the logo, a menu item, anything pointing at a bare url. Read from the query string instead, the choice is kept in session, and `PageController::writingLanguage()` redirects to the localised url itself. Switching back to the writing language works the same way, without a redirect. The links carry `rel="nofollow"`: they are query-string variants of pages the sitemap already declares once per language.

A theme restyles it through `.menu-languages`, `.menu-language-link` and `.menu-language-current`; the language being read is named rather than linked, and dimmed, so the ones that are a choice read as one. The `page_languages(page)` Twig function answers the same list (`locale => url`) for a layout drawing its own.

### Head and sitemap

Every page emits an `hreflang` group naming itself in each language it was **really written in**, built by `PagePublicUrlResolver::resolveAlternates()` and read by the layout through the `page_alternates()` Twig function. A language counts as written when the page's *title* has been translated into it (`PageTranslator::translatedLocales()`), not merely when the site declares it — declaring the whole list is what had `/en/` serve French text under `lang="en"`, which a search engine reads as duplicated content rather than as a translation. A page existing in one language alone therefore emits no group at all, and neither does a collection item's detail view. The same group is carried into `sitemap-site.xml`, where a translated page is declared once per language, each entry carrying the whole group. A localised url of a page nobody translated answers 404 (`PageController::requireTranslated()`), and no menu ever links to one.
...
Every page-level provider walks **one row per page and per language it was written in** (`PageHealthCheckTargets`): read at `/en/pages/nos-ateliers` a page is another title, another prose and another set of links, so it is checked for itself, named in that language's own words on the dashboard, and its row opens that language's screen. The page's own "Health check" tab does the same, each language screen showing the rows of its own url.
...
1. **`CollectionCrudController`** ("Collections" in the dashboard menu) creates/renames/deletes the collections themselves — just a `name` and its `slug` (via EasyAdmin's `SlugField`, auto-filled from `name` but editable, unique site-wide like `Page::$slug`, normalized and de-duplicated server-side by the same recipe as `PageCrudController`). Its index row exposes an "Items" action, and so does its edit screen, which also lists the collection's own items above the form (the first 100, with the total and a link to their index beyond that), each row opening that item.
2. **`CollectionItemCrudController`** manages one collection's items at a time, reached only via that "Items" action or from that panel (`?collectionGroup=<id>`) — never a free-typed field on the item itself, so a typo can no longer silently spawn a brand-new, unrelated collection. The familiar EasyAdmin grid (drag-and-drop reorder included) is filtered to that one collection; a "← Collections" action switches back. An item's own `slug` is unique **within its collection only** (`UNIQ_COLLECTION_ITEM_GROUP_SLUG`, not globally like `Page::$slug`).
...
An item's title and description are translatable (`CollectionItemTranslator`, rows named `site_collection_item` through UiBundle's `ContentTranslator`): once the site declares more than one language, the items index offers a "Translate" action opening a language screen that carries those two fields alone, and both a `collection` block's cards and the item's detail page read them in the language being read. Its image, link, slug and position are the same in every language.
...
That palette lightens `--primary-ink` alone — UiBundle's token for the brand color used as **ink**, where `--primary` stays the **fill** — so every rule writing text, an outline or a rule in the brand color follows in one declaration instead of being lightened token by token: `--link-color`, `--title-color`, the two navbar tokens, `.brand`, `.nav-simple`, `.btn-link`, the burger's focus ring, a `.jumbotron`'s border, a blockquote's bar, the table rules, and everything UiBundle and the satellite bundles paint with it. A deep brand blue reads at 1.43:1 on the `#121212` page, under the 3:1 even large text asks for, while a button and a footer band still need it dark. `theme-color-primary-dark-mode` is followed here too, the mix taking whatever `--primary` resolved to. A site theme painting brand-colored text states `var(--primary-ink)`, never `var(--primary)`. `PrimaryInkRoleTest` locks it, failing on any ink property under `sass/` that reads the fill.

That palette also swaps `--white` with `--black`, which is right for a surface that was white and turns dark, and wrong for ink laid on a ground staying colored in both modes: `--footer-text` and the mobile dropdown's `.menu-label` are a stated `#fff` rather than `var(--white)` for that reason — a design retuning the footer band's ink states its own value here the same way. `DarkThemeTextTokensTest` locks both. Printing is outside the modes altogether: the print sheet forces black ink on white paper with `!important`, pseudo-elements included, so a page read in dark mode doesn't print its own near-black ground.
...
A navbar painted with `--navbar-background: var(--primary)` has three tokens to inverse what would otherwise be invisible on it: `--navbar-site-name-color`/`--navbar-site-tagline-color` for the brand block, and `--navbar-btn-background`/`-background-hover`/`-color` for the single "primary" nav item's pill, whose defaults are UiBundle's `.btn-primary` colors. The first two default to `var(--primary-ink)`/`var(--text)`, which is exactly what a navbar painted in the brand makes invisible.
...
`--footer-group-flex` says what a `menu_group` of the footer takes of its row: a whole line by default, so a footer stacks its social links, then its links, then whatever else. A design laying its footer out in columns — one group per column — gives it a basis the groups can share (`1 1 200px`, say) and picks the *Inline* display style, so the row they sit in reads across rather than down.
...
`--title-color` is the ink every heading is set in — `h1`-`h6`, wherever they are written. It defaults to `var(--primary-ink)`, itself `var(--primary)` on a light page, so nothing moves until a design says otherwise — and a dark page lightens it through that one token rather than through this one. It exists because `--primary` is a **surface** everywhere else — the primary button, a `primary` flat, the footer band — where the white `--button-color` needs it dark: a page wanting its titles in another ink had nowhere to say so short of repainting the brand, and repainting the brand takes the buttons with it. Same pattern as UiBundle's own `--card-accent` / `--flip-card-accent`. What reaches for it is a page laid on a ground that is not the site's: a deep brand color reads at barely more than 1:1 on a near-black page, which is why GalleryBundle's `dark` style sets this token rather than touching `--primary`.

`--link-hover-color` is what a link turns when the mouse passes over it, the underline being what marks the state either way. It defaults to `var(--link-color)`, i.e. no change of ink at all. It exists because `--link-color` was carrying both jobs: a link *at rest* read `--primary` directly, and only the hover rule ever read `--link-color` — so setting the token that names a link's color moved the hover alone, and the lightened value a dark page gives it never reached a link at rest, which went on being painted in the raw brand color. A link at rest reads `--link-color` now, which is a **breaking change** for a site that had set that token to a hover-only accent: point `--link-hover-color` at the old value and set `--link-color` back to `var(--primary-ink)` to keep exactly what it had. `TextInkTokensTest` locks both rules in the compiled stylesheets.
...
Set `site-preconnect` in ConfigBundle to a JSON array of external origins to preconnect to, i.e. `["https://example.com"]`. Useful when `HostedBy`/`MadeBy` logos are served from a third-party domain. Empty by default, so it has no effect unless configured.
...
`fullLayout.html.twig` also pipes the whole document through core-bundle's `absolute_urls(config('site-url'))` filter (`c975l/core-bundle` `^1.17.5`), rewriting every root-relative `src`/`href` against the site's address — a mailbox has no page to resolve `/medias/…` against, so an image or a link left relative is broken the moment the message is sent. Every other email template extends this one, so it's the single place doing it; an unset `site-url` leaves the paths untouched.

`fullLayout.html.twig`'s own copy (no-spam notice, "hello", closing/thanks, "sent by") is four `EmailTemplate` rows — `layout_no_spam`, `layout_hello`, `layout_closing`, `layout_sent_by` — declared by `EmailLayoutTemplateProvider` and seeded by `c975l:ui:email-templates:ensure`, **one row per declared language**. The layout reads them through `email_template_body('layout_hello', {}, locale)`, which resolves the recipient's language first and the site's own next — the locale coming down from `EmailLayoutProvider::wrap()`, which UiBundle hands the language the body itself was rendered in. They are `html` blocks, not `text` ones, the signature carrying a line break and the site's name in bold. `layout_closing` and `layout_sent_by` take a `{{ site }}` placeholder, passed by the layout from `site-name`. An admin writes them on the "Email templates" screen, beside every other email the site sends.

They were four config entries (`email-text-*`) until 02/09/2026. A config entry carries no language, so a bilingual site said them in one language to everyone — which is also why they shipped **empty**: ready-made French paragraphs read as French on a site that isn't. One row per language answers that, so they now ship with real wording in `fr`/`en`/`es`; a site that wants none empties the row, and `ensure` never puts back the wording of a template that already exists.

What the law asks of the footer isn't part of that copy and isn't left to the admin: the sender's identification is read from `site-owner` — the very setting naming the publisher on every legal notice — and a link to the page `url-privacy-policy` names is printed under it, each skipped rather than shown empty while its setting is. The `email-footer` menu an admin builds is rendered *above* them, never instead. A prospection email owes a way out (article L34-5 CPCE) that a transactional one doesn't: the `emailUnsubscribe` block is the hook where each such email places its own.
