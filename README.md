...
The navbar carries a **language menu** (`General:Languages`, rendered inside the bar's items so it follows the mobile dropdown, and on the fallback bar too). It offers the languages the page being read was really written in — the same list the `hreflang` group is built from — so it never links to a url that answers 404, and it renders nothing at all on a site declaring one language, on a page nobody translated, or on a screen with no `Page` behind it. A template declares the `Page` the menu rewrites the url of by setting `navbarPage`, which `pages/page.html.twig` leaves null on a collection item's detail view — the parent Page's languages name the parent's url, not the item's — and in preview, whose public urls would take the editor out of it.
