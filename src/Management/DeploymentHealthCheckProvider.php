...
use c975L\SiteBundle\Service\DeploymentClient;
...
class DeploymentHealthCheckProvider implements HealthCheckProviderInterface
{
    ...
    {
        $label = $this->translator->trans('label.health_check_deployment_https', [], 'site');
    ...
    {
        $label = $this->translator->trans('label.health_check_deployment_not_found', [], 'site');
    ...
    private function row(string $url, string $label, string $status, string $translationId, array $params = [], array $details = []): array
    {
        return [
            ...
            'label' => $label,
            'status' => $status,
            'summary' => $this->translator->trans($translationId, $params, 'site'),
            'details' => $details,
        ];
    }
    ...
    private function errorRow(string $url, string $label, string $message): array
    {
        return [
            ...
            'summary' => $this->translator->trans('label.health_check_deployment_call_failed', ['%message%' => $message], 'site'),
