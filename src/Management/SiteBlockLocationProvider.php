...
// Tells UiBundle's site-wide block screens (the "Legal models" list and its drift health check) which Page a
// given Block sits on, and at which public address - the counterpart of SiteBlockEditUrlProvider, which answers
// where it is edited rather than where it is read. Nothing here is legal-specific: any screen listing one kind
// across the whole site gets its locations from this
        ...
        foreach ($this->pageRepository->findByBlockIds($blockIds) as $page) {
            foreach ($page->getBlocks() as $block) {
                ...
                $locations[$block->getId()] = [
                    ...
                    // Null when the page is in the trash or unpublished, or when "site-url" isn't configured
                    // yet: there is no address to send a visitor - nor a health check - to
