...
class PageLinkLocalizer implements InternalLinkLocalizerInterface
{
    ...
    public function localize(string $value): string
    {
        ...
        if (str_contains($value, 'href="')) {
            return (string) preg_replace_callback(
                '#href="([^"]*)"#',
                fn (array $matches): string => sprintf('href="%s"', $this->localizePath($matches[1], $locale)),
                $value
            );
        }

        return $this->localizePath($value, $locale);
    }
    ...
    {
        if (1 !== preg_match(self::PAGE_PATH, $path, $matches)) {
            return $path;
        }
