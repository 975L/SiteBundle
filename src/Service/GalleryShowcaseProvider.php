...
// Shows "articles_slider" and "menu_link" in UiBundle's block gallery (see BlockGalleryController) -
// neither has a BlockFixtureProviderInterface fixture (see BlockFixtureProvider's own comment for why:
// both only render something once resolved against a real Page/route). Rendered here instead, directly
// against the same underlying components/markup with made-up sample data, bypassing that resolution.
class GalleryShowcaseProvider implements GalleryShowcaseProviderInterface
{
    ...
    // Page/route resolution the real render depends on. The gallery suppresses each kind's own regular
    ...
    private function articlesSliderVariant(): string
    {
        ...
        return $this->twig->render('@c975LUi/components/Slider/Slider.html.twig', [
            'slides' => $slides,
            'id' => 'gallery-articles-slider-preview',
            'duration' => 3500,
