...
// Same icon as UiBundle's ea-sortable.js drag handle (block reordering) - duplicated rather than
// imported cross-bundle, in line with each c975L bundle staying self-contained. No width/height,
// deliberately - EasyAdmin's own icons don't set them either, relying on its global ".icon svg" CSS
// to size every icon consistently; hard-coding one here would make this the odd one out.
...
// Mounted automatically on <body> by controllers-admin.js - no layout override needed. Only ever finds
// rows on CollectionEntryCrudController's own index (see collection_entry_crud_index.html.twig, the only
// template that renders [data-group] on each <tr>), so it's a no-op everywhere else in the back-office.
export default class extends Controller {
    ...
    // A small grip prepended to the "position" cell, the only column whose value the drag actually
    // changes - dragging is only armed on its mousedown, so the rest of the row keeps working normally
    // (EasyAdmin's own default-row-action still opens the edit page on a plain click).
    addHandle(row) {
    ...
    persist(row) {
        ...
        // Keeps the visible number in sync immediately - the server-side position it now maps to would
        // otherwise only show up after a manual reload
