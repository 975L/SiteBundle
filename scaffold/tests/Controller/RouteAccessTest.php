...
class RouteAccessTest extends FunctionalTestCase
{
    ...
    {
        // Authenticated as admin: exercises real controller logic (including role-gated routes like
        // page_preview) instead of being deflected by a login redirect, and stays safe if
        // "site-maintenance" is enabled (see FunctionalTestCase)
    ...
    //No per-route expectation to maintain: /management is already checked more strictly by ManagementAccessTest
    //(must redirect to login when anonymous, not just avoid crashing), and real content is checked by ContentAccessTest.
    public function testNoRouteCrashes(): void
    {
        ...
        foreach ($routes as $name => $route) {
            ...
            // Re-authenticates before every request: a previous route in this same loop may have
            // logged the client out (app_logout is a route like any other here), which would
            // otherwise make every route after it fail under "site-maintenance" for the wrong reason
