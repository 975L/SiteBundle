...
- **A collection item is translatable**, `CollectionItemTranslator` naming its rows `site_collection_item`: its title and its description are laid over the card a "collection" block prints and over the detail page it opens, its image, link, slug and place being the same in every language (11/09/2026)
- Its screen gains the language tabs and a "Translate" action, through CoreBundle's `ContentLocaleScreen` (11/09/2026)
- A translation written there empties the collection's cached cards, `CollectionCacheInvalidationListener` watching the translation row the screen writes rather than the item it leaves untouched (11/09/2026)
- `PageTranslationPurgeListener` takes an item's translations away with the item (11/09/2026)
- **It purges for real**, the id read on `preRemove`: Doctrine nulls it before `postRemove`, so a deleted page always left its translations behind (11/09/2026)
- **A duplicated page carries its translations**: the page's own texts, every block and every picture say in the copy what they said in the original, through CoreBundle's `TranslationCopier` (11/09/2026)
- **The default pages are written in every language the site declares**, the English and Spanish sets `DefaultPagesImporter` already held being stored as the translations of the pages it seeds - matched on the legal model or the form a page holds, a slug being spelled differently in each language (11/09/2026)
- Written only on a page still carrying the words it was seeded with and saying nothing yet in that language: a title an admin rewrote stays theirs, and so does a translation they typed (11/09/2026)
- Requires `c975l/core-bundle` `^1.28.0`, which is what ships the translatable json collections (10/09/2026)
- The translation health check counts the repeated texts of a block too - a FAQ's questions, a grid's cards (10/09/2026)
- A menu's translation screen offers those repeated texts as well, one field per entry: a text the health check reported untranslated had no field to be translated in (11/09/2026)
- The page translation walkthrough names those repeated texts as well, written entry by entry inside the block rather than on a screen of their own (10/09/2026)
- **A language screen carries the whole edit screen**, not the texts alone: the group acting on every language is reproduced on it, mapped, so writing a slug or a priority from the English screen writes it for the French one too (10/09/2026)
- That group is painted apart, on the `fieldset-all-languages` class CoreBundle's back-office stylesheet draws (10/09/2026)
- The QR code is now the last field of the "in this language" group on both screens, rather than the last of the common one on the writing screen (10/09/2026)
...
- **A demo site is seeded in every language it declares**: its pages, their titles, the two alerts, the hero, the text sections and the collection block are written from "site" keys the bundle already ships in French, English and Spanish (see CoreBundle's `DemoFixtureTranslator`) (10/09/2026)
- A rich text is stored inside the very box the block holds it in, so a translation and the text it stands for are the same shape (10/09/2026)
- A collection item is seeded in the writing language alone, the demo dataset holding no "site" key for one (10/09/2026)
- **The language menu is one field rather than a row of links**, on the shape run.as has carried for years: a GET form whose select names the language, the shared path being its action - a site declaring a third language used to take a line of the bar with it, and a fourth would take two (10/09/2026)
- It works with no javascript at all - pick, then press the button; where javascript runs, the "languages" Stimulus controller sends the choice on the change and takes the button away (10/09/2026)
- What the visitor is reading under - a page number, a filter, a sort order - rides along as fields, a GET submission dropping the action's own query string (10/09/2026)
- It takes back the measure UiBundle gives every form - `min(--form-width, 100%)` and an auto margin: three words in a navigation bar were being given 70vw of it, which pushed the menu off the right edge it is aligned on (10/09/2026)
- A crawler is handed nothing to follow, where it used to be handed one query-string variant per language and told not to follow it (10/09/2026)
- **The language menu and the language tabs name a language as a label names it** - "Français", "Español" - reading CoreBundle's `language_name` where they read Twig's `locale_name`, which gives the lowercase form a sentence uses (10/09/2026)
- The "in this language" group names the language on screen rather than the one the site is written in (10/09/2026)
- **The front-end "edit this block" button opens the language being read**, `/en/pages/...` leading to `?contenu=en` rather than to the writing language's screen - where the editor would have overwritten the source text (10/09/2026)
- A block owned by another bundle - a share-buttons panel, a form, a legal model - keeps opening its own screen, none of them having a language yet (10/09/2026)
- **The language menu now shows on a screen with no `Page` behind it**: a shop listing, a product sheet, a campaign, a basket - `General:Languages` falls back on `screen_languages()`, a page keeping the languages it was really translated into (10/09/2026)
- **A menu item pointing at another bundle's route is written in the language being read only where that route answers in it**: a shop category or a campaign not translated yet has a localised route that matches and answers 404, and the item kept pointing at it (10/09/2026)
- The rule the page items already followed, now read off the `locales` a linkable route declares (10/09/2026)
- A route with no localised twin - a basket, a token url - is generated exactly as it always was (10/09/2026)
- **This site's own page links follow the language being read**, `PageLinkLocalizer` applying to a card, a call to action and a word linked inside a rich text the rule `MenuExtension` already applied to a menu item (10/09/2026)
- Only a page really written in that language: a localised url answers for nothing else, so an untranslated page keeps the writing language's link rather than one the visitor lands on a 404 from (10/09/2026)
- A query string and an anchor travel with the link; an external url, a mailto, a bare anchor and another bundle's path are given back untouched (10/09/2026)
- `PagePublicUrlResolver::resolve()` takes a language, and `resolveAll()` gives one url per language a page was written in - what a health check walks (10/09/2026)
- `PageEditUrlResolver::resolve()` takes a language too, opening the screen that language is written on (10/09/2026)
- **Every page-level check now walks one row per page and per language it was written in**, `PageHealthCheckTargets` holding that walk once for the five providers that shared it (10/09/2026)
- A page read at `/en/pages/...` is another title, another prose and another set of links, so it earns a row of its own (10/09/2026)
- PageSpeed, mixed content, W3C HTML, W3C CSS and the dev-profile paths all read from it (10/09/2026)
- Each row is labelled with the language in its own words, "Nos ateliers" and "Our workshops (English)" being two rows of the same dashboard (10/09/2026)
- And each links back to the screen that language is written on, `?contenu=en`, rather than to the writing language's (10/09/2026)
- The unconfigured-site-url guard is said once there rather than in four providers (10/09/2026)
- **A language screen carries that language's own bilan**, `PageHealthCheckPanelType` taking a `content_locale` the way `PageQrCodeType` already did - the rows being stored by url, and that language read at one of its own (10/09/2026)
- Only PageSpeed's quota grows with the number of languages (10/09/2026)
- The translation bilan itself stays one row per page, stored under the writing language's url: how far a page has been translated is one answer for the whole page, not one per language (10/09/2026)
- `PageController` delegates its three language rules to CoreBundle's `LocalizedRouteNegotiator`, and its routes read `c975l_config.locales_pattern` (10/09/2026)
- `PageRepository::findAllOrdered()` states what it gives back (10/09/2026)
- The "edit this block" button reads the `_locale` route attribute rather than `getLocale()`, so a preview no longer opens the session's language (10/09/2026)
- A localised link to the home page lands on the site root, `PageLinkLocalizer` going through `PagePublicUrlResolver` rather than through the router (10/09/2026)
- `PageHealthCheckTargets` and `PageDevProfilePathProvider` preload the translations, one query per language rather than one per page (10/09/2026)
- A content-quality row carries the language it was read in (`PageLocaleSource`), so its "go to the block" links open that language's screen (10/09/2026)
- **The page translation parcours walks the group acting on every language**, one step between the language bar and the title (10/09/2026)
- It walks the sharing summary too, a page's fourth translatable field (10/09/2026)
- The language screen's help text says which fields reach beyond the language on screen (10/09/2026)
- The health-check step says each language screen carries the bilan of its own url (10/09/2026)
- The translation parcours names the site's own "edit this block" button as a way in (10/09/2026)
- The README and the `c975l-site-pages`/`c975l-site-seo`/`c975l-site-menus` skills describe the localised links and the per-language rows (10/09/2026)
- **An item detail confirms its language before its blocks render**, so their links are read in it (10/09/2026)
- `PageController`'s gates and language negotiation move to one `gate()` both the page and the item detail pass through (10/09/2026)
- `PageTranslator::value()` reads one translated field through the cache `preload()` fills, where `all()` cost a query per page and per language (10/09/2026)
- `PageLinkLocalizer` remembers the page behind a slug for the request, a rich text paying one query per internal link before (10/09/2026)
- The guided visit narration is shipped in Spanish, `site_narration.es.xlf` carrying the 84 keys (10/09/2026)
- A test ties the "in this language" step's `highlight` to the class the controller sets, the only selector of the bundle nothing checked (10/09/2026)
- Tests cover the language a block locator is given, the language a health-check panel is asked for, and a block's repeated texts (10/09/2026)
- The page translation walkthrough's blocks step points at a marker `PageCrudController` sets (11/09/2026)
- The collection walkthrough names the list's "Translate" button (11/09/2026)
- Tests tie both language bars' markers to the templates writing them, and cover `CollectionItemTranslator` (11/09/2026)
...
- **This bundle's own javascript is now run rather than read**, through the harness `c975L/UiBundle` ships (`Testing\JsCase`, hence the `c975l/core-bundle` floor at `^1.21` and `chrome-php/chrome` in `require-dev`): 18 scenarios over its three scripts, where it had none. All this bundle declares is where its assets are (`tests/Assets/JsCase`) - its controllers are then served beside UiBundle's in a real headless Chrome, and a scenario reads back what they made of the DOM (02/09/2026)
- The guarantee that earns it is the one `sitemap-fields.js` is written around: a locked field is `readonly` and never `disabled`, because a disabled control is not submitted and every save of a non-referenced page would wipe its frequency and its priority. A `FormData` built off the locked form now says so, where the file could only be read for the attribute it avoids (02/09/2026)
- `publication-switch.js` is held to the same over an index row: an unpublished row arrives unreferenced and locked whatever the database still holds, is given its switch back when published again without being answered for, and a list not showing the reference column is simply left alone (02/09/2026)
- `SitemapFieldsLockTest` was narrowed to what a scenario cannot see rather than kept alongside: the readonly, the aria, the tab order and the dimming are now proven by running the controller, and what stayed is the absence of any style written from script - dropped silently by the back-office nonce - the absence of a true `disabled`, and the one cross-bundle fact a scenario mounting its own markup can never establish, that `.ui-field-locked` is actually painted by UiBundle's `management.css` (02/09/2026)
- **The four sentences wrapping every e-mail leave the configuration**: `email-text-no-spam`, `email-text-hello`, `email-text-closing` and `email-text-sent-by` were config entries the layout read through `config()` - and a config entry carries no language, so a bilingual site said them in one language to everyone. Each is now an `EmailTemplate` of its own, one row per declared language (`EmailLayoutTemplateProvider`, seeded by `c975l:ui:email-templates:ensure`), written on the screen where the rest of a site's e-mails are written. `fullLayout.html.twig` reads them through `email_template_body()`, and the `%site%` placeholder becomes the `{{ site }}` a template resolves at render time (02/09/2026)
- They now ship with real wording in `fr`/`en`/`es`, where the config entries deliberately shipped empty: what made ready-made paragraphs wrong was that one value served every language, which one row per language answers. A site wanting none empties the row, and `ensure` never rewrites a template that already exists (02/09/2026)
- **The site's tagline leaves the configuration too**: `site-tagline` was a config entry the navigation bar read through `config()`, so it too said one thing in one language. It is now the blocks of a fifth menu location, `navbar-brand` - the line under the site's name - written in the menus screen and translated like every block. `.menu-site-tagline` keeps its markup and its place; the section gutter and the lead-in paragraph's own scale and accent bar are turned off there through their custom properties, a block arriving with a page section's markup (02/09/2026)
- **New `c975l:site:content:adopt-config-texts`**, which moves what a site already wrote into the row of its own language - and the tagline into the menu it creates for it - then deletes the entries. It never deletes a text with nowhere to go: run before `c975l:ui:email-templates:ensure` has created the rows, it reports and leaves them. A one-off, meant to be taken out with its line in the deployment once the fleet has run it (02/09/2026)
- **`c975l:site:pages:import-defaults` seeds one set of pages, not one per language**: a page is one page in every language - one row, one set of blocks, its other languages living beside it (see `PageTranslator`) - so a bilingual site was given two legal notices pointing at the same model, and, on `contact`, the one slug the French and English sets share, two rows the flush refused with a duplicate key. The whole command went down with it, on every site declaring two languages. The set imported is the site's own language's, falling back on the first declared one this bundle ships pages for; the legal models were already resolved per locale at render time, so nothing is lost (02/09/2026)
- **Two steps pointed at a TomSelect wrapper the back office never draws**: `#Page_changeFrequency` and `#Menu_style` are plain `ChoiceField`s, rendered as native selects - visible, and pointable directly. Only an association asking for `autocomplete()` is wrapped, and its id carries the suffix anyway; the guard test asserted the opposite and let the two through (02/09/2026)
- **One page is seeded already in the bin**: a bin that has never been used shows an empty screen, where the two things it is for - putting a page back, or removing it for good - both have nothing to act on. The guided project's last step pointed at a button no row carried, its own fourth step having emptied the bin one line above (01/09/2026)
- **Every provider listing the site's pages is now exhaustive** (`w3c-html`, `w3c-css`, `content-quality`, `mixed-content`, `pagespeed`, `translations`): a page moved to the bin or unpublished leaves `PageRepository::findAllOrdered()`, so nothing ever came to replace its last row and the dashboard kept showing that verdict as current, dated by a run that never checked it - `HealthCheckRunner` now drops the rows of a url the run no longer returns (01/09/2026)
- `SiteMediaUsageProvider` marks the usages held by a page in the bin, which is what lets UiBundle's media library leave those medias out of its gallery and `svg-fonts` skip them - reported rather than dropped, so the media stays undeletable (01/09/2026)
- A site url that is missing, or not https for `mixed-content`, throws instead of returning an empty list: an exhaustive kind reads an empty run as "every stored row is stale" and would have cleared its whole history on a misconfigured install (01/09/2026)
- The fixtures, the Readme's preconnect example and `CollectionItemImportCommand`'s comment no longer name the maintainer's own sites, `example.com` and a neutral project standing in their place (01/09/2026)
- **The four sentences wrapping an e-mail are `html` blocks, not `text` ones**, whose line breaks and bold site name used to go out as visible tags (`c975l/core-bundle` `^1.21.1`) (02/09/2026)
- `EmailLayoutProvider` passes the language the body was rendered in down to those four sentences, which followed whichever row the database returned first (02/09/2026)
- `c975l:site:content:adopt-config-texts` writes the old rich-text entries into `html` blocks, and leaves the tagline entry in place when the `navbar-brand` menu already carries blocks (02/09/2026)
- New guided projects `site-page-translation` and `site-menu-translation`, plus two steps writing the baseline under the site's name (02/09/2026)
- New `NarrationCatalogueTest`, a narration key missing from a locale showing nowhere until a film is shot (02/09/2026)
- `DefaultPagesImporter::getDefinitions()` split into one method per locale (02/09/2026)
