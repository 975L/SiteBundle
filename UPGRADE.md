...
**`email-text-legal` is gone.** The setting asked each site to write, in the back office, who its e-mails come from — the very thing `site-owner` already says on every legal notice — and no site had ever filled it in, so every e-mail left identifying nobody. `fullLayout.html.twig` reads `site-owner` now, and prints under it a link to the page `url-privacy-policy` names; the `email-footer` menu an admin builds still renders above both. **Run `php bin/console c975l:config:prune --force`** to take the orphan row away (without `--force` first, to see what it would remove). A site that had written something in `email-text-legal` should move it to `site-owner` **before** pruning; one that overrode `emails/fullLayout.html.twig` has the identification block to bring into its copy, or its e-mails keep carrying none.
