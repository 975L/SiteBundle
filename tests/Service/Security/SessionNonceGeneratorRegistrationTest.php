...
class SessionNonceGeneratorRegistrationTest extends TestCase
{
    ...
    {
        $services = Yaml::parseFile(self::CONFIG_DIR . '/services.yaml')['services'];

        $this->assertArrayNotHasKey(SessionNonceGenerator::class, $services);
    }

    // The file imported by c975LSiteBundle::loadExtension only when the interface exists carries the decoration
