...
{
    // Twig functions registered by SocialBundle alone, a bundle no composer.json of this ecosystem requires - CoreBundle only suggests it
    ...
    public function testNoTemplateCallsASuggestedBundleFunction(string $function): void
    {
        ...
        $this->assertSame(
            ...
            $offenders,
            sprintf(
                '"%s" is registered by SocialBundle alone, which is only ever suggested: %s would fail to compile on a site not installing it. Include the bundle\'s own template with "ignore_missing" instead.',
                $function,
                implode(', ', $offenders)
            )
        );
    ...
    // Requiring it back would make the rule above pointless, and silently re-tie every consuming site to it - the suggestion itself sits in CoreBundle, whose layout.html.twig is the one including the band
    public function testSocialBundleIsNotRequired(): void
    {
